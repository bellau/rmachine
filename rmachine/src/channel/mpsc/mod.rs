...
struct SenderInner<T> {
    inner: Rc<Inner<T>>,
    sender_task: Rc<RefCell<SenderTask>>,
    maybe_parked: bool,
}
...
struct Inner<T> {
    ...
    parked_queue: RefCell<VecDeque<Rc<RefCell<SenderTask>>>>,
...
fn channel2<T>(buffer: Option<usize>) -> (SenderInner<T>, Receiver<T>) {
    let inner = Rc::new(Inner {
        ...
        parked_queue: RefCell::new(VecDeque::new()),
    ...
    let tx = SenderInner {
        inner: inner.clone(),
        sender_task: Rc::new(RefCell::new(SenderTask::new())),
        maybe_parked: false,
    };
...
impl<T> SenderInner<T> {
    ...
    fn park(&mut self) {
        {
            let mut sender = self.sender_task.borrow_mut();
            sender.task = None;
            sender.is_parked = true;
        }

        let t = self.sender_task.clone();
        self.inner.parked_queue.borrow_mut().push_back(t);

        self.maybe_parked = self.inner.state.borrow().is_open();
    }
    ...
    fn poll_unparked(&mut self, cx: Option<&mut Context<'_>>) -> Poll<()> {
        if self.maybe_parked {
            let mut task = self.sender_task.borrow_mut();
...
impl<T> Receiver<T> {
    ...
    pub fn close(&mut self) {
        if let Some(inner) = &mut self.inner {
            ...
            while let Some(task) = inner.parked_queue.borrow_mut().pop_front() {
                task.borrow_mut().notify();
            }
        }
    }
    ...
    fn unpark_one(&mut self) {
        if let Some(inner) = &mut self.inner {
            if let Some(task) = inner.parked_queue.borrow_mut().pop_front() {
                task.borrow_mut().notify();
            }
        }
    }
